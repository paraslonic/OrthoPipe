...
my ($script_name,$script_version,$script_date,$script_years) = ('fasta-splitter','0.2.1','2014-08-25','2012-2014');
my $start_time = time;

my ($opt_n_parts,$opt_part_size,$opt_measure,$opt_line_len,$opt_eol,$ver,$help);
GetOptions('n-parts=i'     => \$opt_n_parts,
           ...
           'version'       => \$ver,
           'help'          => \$help);
while (defined(my $i = first { $ARGV[$_] =~ /^-/; } keys @ARGV))
{
    if ($i < scalar(@ARGV)-1)
    {
            if($ARGV[$i] == '-n-parts-total')      { $opt_n_parts   = int($ARGV[$i+1]); $opt_measure = 'all'; }
            if($ARGV[$i] == '-n-parts-sequence')   { $opt_n_parts   = int($ARGV[$i+1]); $opt_measure = 'seq'; }
            if($ARGV[$i] == '-part-total-size')    { $opt_part_size = int($ARGV[$i+1]); $opt_measure = 'all'; }
            if($ARGV[$i] == '-part-sequence-size') { $opt_part_size = int($ARGV[$i+1]); $opt_measure = 'seq'; }
            if($ARGV[$i] == '-line-lengthe')        { $opt_line_len  = int($ARGV[$i+1]); }
            if($ARGV[$i] ==  '-eol')                { $opt_eol       = int($ARGV[$i+1]); }
        splice @ARGV, $i, 2;
    }
}

my $ver_str = "$script_name, version $script_version, $script_date\nCopyright (c) $script_years Kirill Kryukov\n";
my $help_str = qq{Usage: $script_name.pl \[options\] <file>...
Options:
